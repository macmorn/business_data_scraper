...
EU Company Research Pipeline — a resumable, multi-stage pipeline that extracts company names from PDF/Excel supplier lists, enriches them through multiple data sources (Northdata, OpenCorporates, GLEIF, Pappers, Brave Search, Codex AI), and exports enriched CSV files.
...
| S05 | `stages/s05_ai_enrich.py` | yes | Codex AI disambiguation + CEO career summaries |
...
- **`claude_ai.py`** — Uses `Codex-agent-sdk` (not the Anthropic API directly) with web search tool
